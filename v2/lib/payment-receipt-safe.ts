...
function paymentMethod(context: ReceiptContext) {
  ...
  return method ? String(method).replace(/_/g, ' ') : 'Not recorded';
}
...
async function buildPdf(context: ReceiptContext, number: string, requestUrl: string) {
  ...
  // The supplied A4 letterhead is the complete background. No custom design is drawn.
  page.drawImage(letterhead, { x: 0, y: 0, width: PAGE_WIDTH, height: PAGE_HEIGHT });
