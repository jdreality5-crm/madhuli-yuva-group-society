...
async function loadContext(paymentId: string, societyId: string): Promise<ReceiptContext | null> {
  ...
  const [owners, societies, events, accounts, bills] = await Promise.all([
    ...
    rest<any[]>('Society', { select: 'id,name,address,city,state,pincode,reportFooter', id: `eq.${societyId}`, limit: '1' }),
...
async function buildPdf(context: ReceiptContext, number: string) {
  ...
  page.drawText('Authorized Signature', { x: 350, y: 220, size: 10, font: regular, color: dark });
  page.drawLine({ start: { x: 340, y: 205 }, end: { x: 515, y: 205 }, thickness: 0.8, color: gold });
  page.drawText('Avnish Patel', { x: 340, y: 180, size: 11, font: bold, color: maroon });
  page.drawText('President', { x: 340, y: 162, size: 10, font: regular, color: dark });
  return pdf.save();
}
