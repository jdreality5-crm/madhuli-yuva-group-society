...
type P = { id: string; amountPaise: string; status: string; transactionId?: string | null; screenshotUrl?: string | null; rejectionReason?: string | null; expiresAt: string; paymentAccount: { displayName: string; upiId?: string | null; purpose: string; qrImageUrl?: string | null } };
...
export default function Payments() {
  ...
  async function load() {
    try {
      const [ar, pr] = await Promise.all([fetch('/api/admin/payment-accounts'), fetch('/api/payments')]);
  ...
  useEffect(() => {
    if (!current) return;
    const tick = () => setSeconds(Math.max(0, Math.floor((new Date(current.expiresAt).getTime() - Date.now()) / 1000)));
    tick();
  ...
  async function start(e: React.FormEvent) {
    ...
    setCurrent({ ...x.payment, paymentAccount: a.find(v => v.id === account) || x.payment.paymentAccount });
    setTx('');
    setShot('');
    setMsg('Payment session created. Complete the UPI payment using the QR code or your UPI app, then submit the transaction ID and screenshot here.');
    // Do not navigate to upi:// automatically: desktop browsers show “This page couldn’t load”.
  }
  ...
  async function submit() {
    ...
    setMsg('Payment proof submitted. Your status is now Pending verification. Master Admin / Accountant will review it.');
    setCurrent(null);
    setTx('');
    setShot('');
    await load();
  ...
  return <main className="main">
    ...
    <form className="card finance-form grid" onSubmit={start}>
      <div className="section-head"><div><p className="eyebrow">NEW PAYMENT</p><h2>{billId ? 'Pay Bill' : 'Make a payment'}</h2><p className="section-subtitle">Choose the society receiver, enter the amount, then complete payment in your UPI app.</p></div></div>
    ...
    {current && <section className="card payment-processing">
      <div className="section-head"><div><p className="eyebrow">PAYMENT SESSION</p><h2>Complete payment</h2><p className="section-subtitle">Complete the UPI payment before the session expires, then submit proof.</p></div></div>
      ...
      {current.paymentAccount.qrImageUrl && <img src={current.paymentAccount.qrImageUrl} alt="UPI QR" style={{ width: 240, height: 240, objectFit: 'contain' }} />}
      {current.paymentAccount.upiId && <p><b>UPI ID:</b> {current.paymentAccount.upiId}</p>}
      {current.paymentAccount.instructions && <p>{current.paymentAccount.instructions}</p>}
      <p>After completing payment in Google Pay, PhonePe, Paytm or another UPI app, return here.</p>
      ...
      <button className="btn btn-primary" disabled={!tx.trim() || !shot || seconds === 0} onClick={submit}>Submit Payment Proof</button>
    </section>}
