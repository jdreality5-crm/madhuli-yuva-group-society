...
export default function ProfilePage(){
  ...
  const residence = unit?.property?.type==='APARTMENT'
    ...
    : unit?.property?.type==='TENAMENT'
      ...
      : null;
  return <main className="main"><div className="page-header"><div><h1>My Profile</h1><p>Manage your personal details and profile picture.</p></div></div>{message&&<div className="alert">{message}</div>}
    <section className="card profile-card"><div className="profile-top"><div className="avatar">{profile.profileImageUrl?<img src={profile.profileImageUrl} alt="Profile"/>:profile.name.slice(0,1).toUpperCase()}</div><div><h2>{profile.name}</h2><p>{profile.role.replace('_',' ')}</p><div className="actions"><label className="btn btn-secondary">{uploading?'Uploading…':'Change picture'}<input hidden type="file" accept="image/jpeg,image/png,image/webp" disabled={uploading} onChange={e=>{const f=e.target.files?.[0];if(f)upload(f)}}/></label>{profile.profileImageUrl&&<button className="btn btn-secondary" disabled={uploading} onClick={remove}>Remove</button>}</div><small>JPG, PNG or WebP · maximum 3 MB</small></div></div>
    ...
    {unit&&residence&&<div className="resident-box"><h3>Residence</h3><p><strong>{residence}</strong></p><p>{unit.property.name} · {profile.residentType||'Resident'}</p></div>}
