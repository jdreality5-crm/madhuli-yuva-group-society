...
export async function POST(req: Request) {
  try {
    ...
    if (!user || user.role !== 'OWNER') return NextResponse.json({ error: 'No resident verification request was found for this email.' }, { status: 404 });
    if (user.approvalStatus === 'REJECTED') return NextResponse.json({ error: 'This registration was rejected. Please contact the society administrator.' }, { status: 403 });
    if (user.emailVerified) return NextResponse.json({ error: 'This email is already verified. Please login after approval.' }, { status: 409 });
    if (user.status !== 'INACTIVE') return NextResponse.json({ error: 'No pending resident verification was found for this email.' }, { status: 404 });
    ...
    const otp = newOtp(); const tokenHash = await hashOtp(otp);
    const token = await prisma.verificationToken.create({ data: { userId: user.id, tokenHash, expiresAt: new Date(Date.now() + OTP_TTL_MS) } });
    try { await sendVerificationOtp(email, otp); } catch (error) { await prisma.verificationToken.updateMany({ where: { id: token.id, consumedAt: null }, data: { consumedAt: new Date() } }); throw error; }
