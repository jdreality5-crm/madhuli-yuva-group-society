...
export async function POST(req: Request) {
  try {
    ...
    const user = await prisma.$transaction(async tx => {
      ...
      if (unitId) await tx.propertyUnit.update({ where: { id: unitId }, data: { residentUserId: created.id, residentType: residentType || 'OWNER', ownerName: body.name, ownerMobile: mobile, ownerEmail: email } });
      return created;
    });
