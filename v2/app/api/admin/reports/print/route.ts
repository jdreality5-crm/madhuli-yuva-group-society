...
export async function GET(req: Request) {
  try {
    ...
    const totalIncome = incomeRows.reduce((sum, row) => sum + BigInt(String(row.amountPaise || 0)), 0n);
    const totalExpense = expenseRows.reduce((sum, row) => sum + BigInt(String(row.amountPaise || 0)), 0n);
    ...
    const rows = (items: any[]) => items.map((row) => `<tr><td>${esc(new Date(row.date).toLocaleDateString('en-IN'))}</td><td>${esc(row.category || 'Other')}</td><td>${esc(row.description || '—')}</td><td>${esc(row.receivedFrom || row.paidTo || '—')}</td><td class="amount">${esc(money(String(row.amountPaise || 0)))}</td></tr>`).join('');
