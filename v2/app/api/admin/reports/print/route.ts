...
const M = 38;
const CW = W - M * 2;
const CONTENT_TOP = 612;
const CONTENT_BOTTOM = 286;
...
type ReportData = { societyName: string; signatory: string; title: string; period: string; income: Row[]; expenses: Row[]; letterhead: PDFImage | null };
...
function draw(page: PDFPage, value: unknown, x: number, y: number, font: PDFFont, size: number, color = INK, maxWidth?: number) {
  ...
  if (maxWidth) {
    while (rendered.length > 3 && font.widthOfTextAtSize(rendered, size) > maxWidth) rendered = `${rendered.slice(0, -4)}...`;
  }
...
function frame(page: PDFPage, data: ReportData, regular: PDFFont, bold: PDFFont, number: number) {
  ...
  draw(page, `Page ${number}`, W - M - 42, 636, regular, 7.2, MUTED, 42);
}
...
function tableHead(page: PDFPage, columns: Column[], y: number, bold: PDFFont) {
  ...
  columns.forEach((column) => {
    draw(page, column.label, x + 3, y + 1, bold, 5.7, rgb(1, 1, 1), column.width - 6);
    x += column.width;
  });
  return y - 18;
}
...
function newPage(pdf: PDFDocument, data: ReportData, regular: PDFFont, bold: PDFFont, number: number) {
  ...
  frame(page, data, regular, bold, number);
