...
export async function GET(req: Request) {
  try {
    ...
    const eventMap = new Map<string, { income: bigint; expense: bigint }>();
    for (const row of income) { const key = row.event?.title || 'Other / General'; const current = eventMap.get(key) || { income: 0n, expense: 0n }; current.income += row.amountPaise; eventMap.set(key, current); }
    for (const row of expense) { const key = row.event?.title || 'Other / General'; const current = eventMap.get(key) || { income: 0n, expense: 0n }; current.expense += row.amountPaise; eventMap.set(key, current); }
    const eventRows = Array.from(eventMap.entries()).map(([title, values], index) =>
      `<tr><td>${index + 1}</td><td>${esc(title)}</td><td>${money(values.income)}</td><td>${money(values.expense)}</td><td class="amt">${money(values.income - values.expense)}</td></tr>`,
    ).join('');
