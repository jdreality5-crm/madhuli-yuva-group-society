...
export async function PATCH(req: Request) {
  try {
    ...
    const result = await prisma.$transaction(async tx => {
      const user = await tx.user.findFirst({ where:{id:body.id,societyId:session.societyId,role:'OWNER'}, select:{id:true,email:true,approvalStatus:true,unitId:true} });
      if (!user) return { kind:'NOT_FOUND' as const };
      if (user.approvalStatus !== 'PENDING') return { kind:'ALREADY_PROCESSED' as const };
      const updated = await tx.user.update({ where:{id:user.id}, data:{approvalStatus:body.action==='APPROVE'?'APPROVED':'REJECTED',status:'INACTIVE'}, select:{id:true,email:true,approvalStatus:true,status:true,emailVerified:true} });
      if (body.action === 'REJECT' && user.unitId) {
        await tx.propertyUnit.updateMany({ where:{id:user.unitId,residentUserId:user.id}, data:{residentUserId:null} });
        await tx.user.update({ where:{id:user.id}, data:{unitId:null,residentType:null} });
      }
      await tx.auditLog.create({ data:{userId:session.id,action:body.action,module:'OWNER_APPROVAL',recordId:user.id,details:`${body.action==='APPROVE'?'Approved':'Rejected'} resident ${user.email}`} });
