...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string; unitId: string }> }) {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      const unit = await tx.propertyUnit.findFirst({
        where: { id: unitId, propertyId: id, property: { societyId: session.societyId } },
      ...
      if (previousUserId && previousUserId !== residentUserId) {
        await tx.user.updateMany({ where: { id: previousUserId, societyId: session.societyId, unitId }, data: { unitId: null, residentType: null } });
      }
      if (residentUserId) {
        await tx.user.update({ where: { id: residentUserId }, data: { unitId, residentType: data.residentType } });
      }

      await tx.auditLog.create({
        data: { societyId: session.societyId, actorUserId: session.id, action: "UPDATE", module: "PROPERTY_UNIT", recordId: unitId, details: JSON.stringify({ propertyId: id, residentType: data.residentType, residentUserId: residentUserId || null, signupEnabled: data.signupEnabled }) },
      });
      return updated;
    });
    ...
    const map: Record<string, [string, number]> = {
      ...
      USER_NOT_FOUND: ["Resident user not found", 404],
      ONLY_RESIDENT_USER: ["Only resident accounts can be linked to a property unit", 400],
