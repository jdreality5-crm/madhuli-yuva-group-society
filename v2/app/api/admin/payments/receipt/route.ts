...
export async function GET(req: Request) {
  try {
    ...
    const rows = await rest<any[]>('Payment', { select: '*', id: `eq.${parsed.data.paymentId}`, societyId: `eq.${session.societyId}`, status: 'eq.VERIFIED', limit: '1' });
    const payment = rows[0];
    if (!payment) return NextResponse.json({ error: 'Verified payment not found.' }, { status: 404 });

    let receiptNumber = payment.receiptNumber || null;
    let receiptStoragePath = payment.receiptStoragePath || null;
    ...
    return NextResponse.json({ error: status === 403 ? 'Organizer access required.' : status === 401 ? 'Authentication required.' : status === 503 ? 'Receipt storage link unavailable.' : 'Unable to prepare receipt download.' }, { status });
  }
}
