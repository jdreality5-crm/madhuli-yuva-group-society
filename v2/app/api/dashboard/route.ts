...
async function safeRest<T>(table: string, params: Record<string, string>, fallback: T): Promise<T> {
  try {
    return await supabaseRest<T>(table, params);
  } catch {
    return fallback;
  }
}
...
export async function GET() {
  try {
    ...
    const societyRows = await safeRest<any[]>('Society', {
      id: 'eq.' + session.societyId,
      select: 'id,name,address,city,state,logoUrl,contact',
      limit: '1',
    }, []);
    ...
    const [upcomingEvents, notices, photos] = await Promise.all([
      safeRest<any[]>('Event', {
        ...
        order: 'date.asc',
        limit: '6',
        select: 'id,title,gujaratiTitle,date,time,location,description,imageUrl',
      }, []),
      safeRest<any[]>('Notice', {
        societyId: 'eq.' + session.societyId,
        status: 'eq.PUBLISHED',
        order: 'date.desc',
        limit: '8',
        select: 'id,title,gujaratiTitle,content,gujaratiContent,date,important,imageUrl',
      }, []),
      safeRest<any[]>('Photo', {
        societyId: 'eq.' + session.societyId,
        order: 'createdAt.desc',
        limit: '12',
        select: 'id,title,fileUrl,altText,albumName,eventId',
      }, []),
    ]);

    const society = societyRows[0]
      ? { ...societyRows[0], logoUrl: await mediaUrl(societyRows[0].logoUrl, session.societyId) }
      : null;
    ...
    if (session.role === 'OWNER') {
      return NextResponse.json({ role: session.role, permissions: session.permissions, society, upcomingEvents: signedEvents, notices: signedNotices, photos: signedPhotos });
    }

    const properties = await safeRest<PropertyRow[]>('Property', {
      societyId: 'eq.' + session.societyId,
      status: 'eq.ACTIVE',
      select: 'id',
    }, []);
    ...
    const [income, expense, propertyUnits, events] = await Promise.all([
      safeRest<any[]>('Income', { societyId: 'eq.' + session.societyId, select: 'amountPaise' }, []),
      safeRest<any[]>('Expense', { societyId: 'eq.' + session.societyId, select: 'amountPaise' }, []),
      propertyIds.length
        ? safeRest<any[]>('PropertyUnit', { propertyId: 'in.(' + propertyIds.join(',') + ')', status: 'eq.ACTIVE', select: 'id' }, [])
        : Promise.resolve([]),
      safeRest<any[]>('Event', { societyId: 'eq.' + session.societyId, select: 'id' }, []),
    ]);
    ...
    return NextResponse.json({
      role: session.role,
      society,
      upcomingEvents: signedEvents,
      notices: signedNotices,
      photos: signedPhotos,
      stats: {
        totalIncome: totalIncome.toString(),
        totalExpense: totalExpense.toString(),
        balance: (totalIncome - totalExpense).toString(),
        flats: propertyUnits.length,
